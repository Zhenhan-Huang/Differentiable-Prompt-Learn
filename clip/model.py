...
class ResidualAttentionBlock_TrainerDev(nn.Module):
    def __init__(
    ...
    def cross_attention(self, q, kv, attn_mask):
        attn_mask = attn_mask.to(dtype=q.dtype, device=q.device) if attn_mask is not None else None
        return self.attn(q, kv, kv, need_weights=False, attn_mask=attn_mask)[0]

    def forward(self, inputs):
        x = inputs[0]
        # alphas: (depth, nctx_max+1)
        alphas = inputs[1]
        ctx_deep = inputs[2]
        counter = inputs[3]
        alph = alphas[self.lid]
        alph = F.softmax(alph.float(), dim=0)
        n_ctx = alphas.size(1)
        ctxs = ctx_deep[self.lid]
        if self.text_layer:
            dtype = (self.attn_mask).dtype
            for cc in range(n_ctx):
                if cc == 0:
                    op_out = alph[cc] * self.cross_attention(
                        self.ln_1(x), self.ln_1(x), self.attn_mask)
                else:
                    ctx_vec = ctxs[cc-1].expand(x.shape[1], -1, -1).permute(1, 0, 2)
                    kv = torch.cat([x[:1], ctx_vec, x[1:]], dim=0)
                    num_ctx = ctx_vec.size(0)
                    seq = x.size(0)
                    mask_extend = torch.zeros(seq, num_ctx, dtype=dtype)
                    cross_attn_mask = torch.cat([mask_extend, self.attn_mask], dim=1)
                    op_out += alph[cc] * self.cross_attention(
                        self.ln_1(x), self.ln_1(kv), cross_attn_mask)
        else:
            for cc in range(n_ctx):
                if cc == 0:
                    op_out = alph[cc] * self.cross_attention(
                    ...
                    ctx_vec = ctxs[cc-1].expand(x.shape[1], -1, -1).permute(1, 0, 2)
                    kv = torch.cat([x, ctx_vec], dim=0)
                    op_out += alph[cc] * self.cross_attention(
                        self.ln_1(x), self.ln_1(kv), self.attn_mask)

        x = x + op_out
        x = x + self.mlp(self.ln_2(x))
        return [x, alphas, ctx_deep, counter]  # return again as a list, so that nn.seq can work
    

class ResidualAttentionBlock_TrainerSubnet(nn.Module):
    def __init__(
        self, d_model: int, n_head: int, attn_mask: torch.Tensor = None,
        design_details=None, text_layer=False, i=0):
        super().__init__()
    ...
    def forward(self, inputs):
        ...
        if self.text_layer:
            dtype = (self.attn_mask).dtype
            ctx_vec = ctxs.unsqueeze(0).expand(x.shape[1], -1, -1).permute(1, 0, 2)
            kv = torch.cat([x[:1], ctx_vec, x[1:]], dim=0)
            num_ctx = ctx_vec.size(0)
            seq = x.size(0)
            mask_extend = torch.zeros(seq, num_ctx, dtype=dtype)
            cross_attn_mask = torch.cat([mask_extend, self.attn_mask], dim=1)
            op_out = self.cross_attention(self.ln_1(x), self.ln_1(kv), cross_attn_mask)
        else:
            ctx_vec = ctxs.unsqueeze(0).expand(x.shape[1], -1, -1).permute(1, 0, 2)
            kv = torch.cat([x, ctx_vec], dim=0)
            op_out = self.cross_attention(self.ln_1(x), self.ln_1(kv), self.attn_mask)

        x = x + op_out
        x = x + self.mlp(self.ln_2(x))
        return [x, None, ctx_deep, counter]  # return again as a list, so that nn.seq can work
    

class Transformer(nn.Module):
    def __init__(
        self, width: int, layers: int, heads: int,
        attn_mask: torch.Tensor = None, text_layer=False, design_details=None
    ):
        super().__init__()
        self.width = width
        self.layers = layers
        ...
        if current_trainer == 'IVLP' or current_trainer == 'VPT':
            prompts_needed = design_details['language_depth']
            self.resblocks = nn.Sequential(*[ResidualAttentionBlock_IVLP(
                width, heads, attn_mask, True, text_layer, i, design_details) if prompts_needed > i
                                             else ResidualAttentionBlock_IVLP(
                width, heads, attn_mask, False, text_layer, i, design_details)
                                             for i in range(layers)])
        elif current_trainer == 'MaPLe':
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_MaPLe(width, heads, attn_mask, design_details, text_layer, i)
                  for i in range(layers)])
        elif current_trainer == 'TrainerSupernet':
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_TrainerDev(
                    width, heads, attn_mask, design_details, text_layer, i)
                  for i in range(layers)])
        elif current_trainer == 'TrainerSubnet':
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_TrainerSubnet(
                    width, heads, attn_mask, design_details, text_layer, i)
                 for i in range(layers)])
        else:
...
class VisionTransformer_TrainerDev(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,
                 design_details):
        super().__init__()
        self.input_resolution = input_resolution
    ...
    def forward(self, x, alphas, ctxs):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        x = torch.cat(
        ...
        x = x.permute(1, 0, 2)  # NLD -> LND
        # Again combine the inputs, so nn.sequential can work
        outputs = self.transformer([x, alphas, ctxs, 0])
        x = outputs[0]
        x = x.permute(1, 0, 2)  # LND -> NLD
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            #TODO: delete this debug snippet
            if trainer == "MaPLe":
            ...
            elif trainer == "TrainerSupernet" or trainer == 'TrainerSubnet':
                self.visual = VisionTransformer_TrainerDev(
                    input_resolution=image_resolution,
                    patch_size=vision_patch_size,
                    width=vision_width,
                    layers=vision_layers,
